...
EOF
    ...
    wget -O $HOME/aztec-testnet.tar.lz4 https://files5.blacknodes.net/aztec/aztec-testnet.tar.lz4
...
check_ports_and_peerid() {
  ...
  PEER_ID=$(sudo bash -c "docker logs \$(docker ps -q --filter ancestor=aztecprotocol/aztec:2.0.2 | head -n 1) 2>&1 | grep -i peerId | grep -o '\"peerId\":\"[^\"]*\"' | cut -d'\"' -f4 | head -n 1")
  if [ -n "$PEER_ID" ]; then
...
EOF
       ...
       ;;
    5) check_rpc_health ;;
    6) echo "⚠️ Delete Node will affect these paths:"; echo "   - $HOME/aztec"; echo "   - $HOME/.aztec/testnet"; echo "   - Docker container: aztec-sequencer"; echo ""; read -p "➡ Do you want to continue? (Y/n): " confirm; if [[ "$confirm" =~ ^[Yy]$ || -z "$confirm" ]]; then read -p "➡ Are you REALLY sure you want to delete these? (Y/n): " confirm2; if [[ "$confirm2" =~ ^[Yy]$ || -z "$confirm2" ]]; then docker stop aztec-sequencer 2>/dev/null || true; docker rm aztec-sequencer 2>/dev/null || true; rm -rf "$HOME/aztec"; sudo rm -rf "$HOME/.aztec/testnet"; echo "✅ Aztec Node deleted."; else echo "❌ Second confirmation failed."; fi; else echo "❌ Delete cancelled."; fi ;;
    ...
    10) echo -e "${CYAN}Exiting...${NC}"; break ;;
    *) echo "Invalid option" ;;
  esac

  read -p "Press Enter to continue..."
done    sudo apt-get install -y ca-certificates curl gnupg lsb-release
    sudo install -m 0755 -d /etc/apt/keyrings
    . /etc/os-release
    curl -fsSL "https://download.docker.com/linux/$ID/gpg" | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg
    sudo chmod a+r /etc/apt/keyrings/docker.gpg
